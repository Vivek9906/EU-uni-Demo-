...
const CATEGORIES = ['General', 'Academic', 'Events', 'Research', 'Student Life']

export function NewsFormClient({ mode, news }: NewsFormClientProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      try {
        const method = mode === 'edit' && news?.id ? 'PUT' : 'POST'
        const url = mode === 'edit' && news?.id ? `/api/admin/news/${news.id}` : '/api/admin/news'
        
        const res = await fetch(url, {
          method,
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ ...form, id: news?.id }),
        })
      ...
      <div style={{ background:'#FFF', borderBottom:'1px solid #E2E8F0', padding:'20px 28px', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
        <div>
          ...
          <h1 style={{ fontSize:22, fontWeight:800, color:'#0F172A', margin:0 }}>
            {mode === 'create' ? 'New News' : 'Edit News'}
          </h1>
        </div>
      </div>
      ...
      <div style={{ padding:'28px', maxWidth:680 }}>
        <form onSubmit={handleSubmit}>
          <div style={{ background:'#FFF', borderRadius:12, border:'1px solid #E2E8F0', padding:'28px' }}>
            ...
            <div style={{ marginBottom:18 }}>
              <label style={{ display:'block', fontSize:12.5, fontWeight:700, color:'#374151', marginBottom:6 }}>Excerpt <span style={{ color:'#DC2626' }}>*</span></label>
              <input required value={form.excerpt} onChange={e => setForm(f => ({ ...f, excerpt: e.target.value }))} style={{ width:'100%', padding:'10px 13px', border:'1.5px solid #D1D5DB', borderRadius:8, fontSize:14, boxSizing:'border-box', fontFamily:'inherit' }} />
            </div>
            ...
            <div style={{ marginBottom:18 }}>
              <label style={{ display:'block', fontSize:12.5, fontWeight:700, color:'#374151', marginBottom:6 }}>Content <span style={{ color:'#DC2626' }}>*</span></label>
              <textarea required rows={6} value={form.content} onChange={e => setForm(f => ({ ...f, content: e.target.value }))} style={{ width:'100%', padding:'10px 13px', border:'1.5px solid #D1D5DB', borderRadius:8, fontSize:14, resize:'vertical', fontFamily:'inherit', boxSizing:'border-box' }} />
            </div>
          ...
          <div style={{ display:'flex', gap:12, marginTop:16, justifyContent:'flex-end' }}>
            ...
            <button type="submit" disabled={isPending} style={{ padding:'10px 24px', background: isPending ? '#94A3B8' : '#1B3A6B', color:'#FFF', border:'none', borderRadius:8, fontWeight:700, cursor: isPending ? 'not-allowed' : 'pointer', fontFamily:'inherit' }}>
              {isPending ? 'Saving...' : mode === 'create' ? 'Create News' : 'Save Changes'}
            </button>
          </div>
        </form>
      </div>
