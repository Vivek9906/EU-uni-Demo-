  ...
  unstable_cache(
    async () => {
      return prisma.program.findFirst({
        where: { slug, level: 'phd', isActive: true },
        select: {
          id: true,
          title: true,
          slug: true,
    ...
    [`program-phd-detail-${slug}`],
    { revalidate: 3600, tags: ['programs'] }
  );

export async function generateStaticParams() {
  return PROGRAMS.phd.map((p) => ({ slug: p.slug }));
}
...
export default async function DynamicPhdPage({ params }: Props) {
  const { slug } = await params;
  const program = await getCachedProgram(slug)();
