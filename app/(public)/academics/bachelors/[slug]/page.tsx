  ...
  unstable_cache(
    async () => {
      return prisma.program.findFirst({
        where: { slug, level: 'bachelors', isActive: true },
        select: {
          id: true,
          title: true,
          slug: true,
    ...
    [`program-bachelors-detail-${slug}`],
    { revalidate: 3600, tags: ['programs'] }
  );

export async function generateStaticParams() {
  return PROGRAMS.bachelors.map((p) => ({ slug: p.slug }));
}
...
export default async function DynamicBachelorsPage({ params }: Props) {
  const { slug } = await params;
  const program = await getCachedProgram(slug)();
